"""
Train Classification Models with BALANCED DATA
Uses SMOTE to create synthetic samples + Class Weights
...
def prepare_balanced_data(sample_size=10, n_timestamps=500):
    """Prepare data with SMOTE balancing and synthetic samples."""
    print("\n" + "="*70)
    print("PREPARING BALANCED CLASSIFICATION DATA WITH SMOTE")
    print("="*70)
...
def plot_confusion_matrices(all_metrics, results_dir):
    ...
    plt.tight_layout()
    save_path = os.path.join(results_dir, 'plots', 'confusion_matrices_balanced.png')
...
"""
    ...
    plt.tight_layout()
    save_path = os.path.join(results_dir, 'plots', 'metrics_comparison_balanced.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"[OK] Saved: {save_path}")
...
def main():
    ...
    print("\n" + "="*70)
    print("VM CPU CLASSIFICATION - BALANCED DATA WITH SMOTE")
    print("="*70)

    # Setup
    results_dir = 'results_classification_balanced'
    os.makedirs(results_dir, exist_ok=True)
    os.makedirs(os.path.join(results_dir, 'models'), exist_ok=True)
    os.makedirs(os.path.join(results_dir, 'plots'), exist_ok=True)

    # Prepare balanced data
    data = prepare_balanced_data(sample_size=10, n_timestamps=500)
    ...
    for model_name in model_names:
        ...
        history = model.fit(
            ...
            epochs=20,
            batch_size=32,
            class_weight=data['class_weights'],  # Apply class weights!
    ...
    results_path = os.path.join(results_dir, 'classification_results_balanced.json')
